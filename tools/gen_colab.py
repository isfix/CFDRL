...
def create_notebook():
    ...
    cells.append(code_cell([
        ...
        "    optimizer.step()\n",
        "    memory.update_priorities(idxs, td.abs().detach().cpu().numpy() + 1e-6)\n",
        "    return loss.item()\n",
        "\n",
    ...
    cells.append(code_cell([
        ...
        "    policy_net.train()\n",
        "    env = TradingEnv(feature_data[:train_end], close_prices[:train_end],\n",
        "                     Settings.SEQUENCE_LENGTH, Settings.TRANSACTION_COST_BPS, SF)\n",
        "    mkt, ts = env.reset()\n",
        "    ep_reward, ep_trades, losses = 0.0, 0, []\n",
        "    max_steps = train_end - Settings.SEQUENCE_LENGTH - 1\n",
        "    pbar = tqdm(range(max_steps), desc=f'Epoch {epoch}/{Settings.EPOCHS}', ncols=110)\n",
        "\n",
        "    for step in pbar:\n",
        "        mt = torch.FloatTensor(mkt).unsqueeze(0).to(device)\n",
        "        tt = torch.FloatTensor(ts).unsqueeze(0).to(device)\n",
